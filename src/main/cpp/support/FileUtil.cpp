...
io_result read_fd(native_fd fd, void* buffer, unsigned int count) {
    return ::_read(fd, buffer, count);
}

io_result write_fd(native_fd fd, const void* buffer, unsigned int count) {
    return ::_write(fd, buffer, count);
}

int close_fd(native_fd fd) {
    return ::_close(fd);
}
#else
using native_fd = int;
using io_result = ssize_t;
constexpr native_fd kInvalidFd = -1;
...
io_result read_fd(native_fd fd, void* buffer, std::size_t count) {
    return ::read(fd, buffer, count);
}

io_result write_fd(native_fd fd, const void* buffer, std::size_t count) {
...
std::string read_text_file(const std::filesystem::path& path) {
    ...
    char buffer[8192];
    while (true) {
        const io_result nbytes = read_fd(fd, buffer, sizeof(buffer));
        if (nbytes < 0) {
            close_fd(fd);
...
bool write_text_file(const std::filesystem::path& path, std::string_view content) {
    ...
    while (remaining > 0) {
        const io_result written = write_fd(fd, data, static_cast<unsigned int>(remaining));
        if (written <= 0) {
            close_fd(fd);
            return false;
        }
